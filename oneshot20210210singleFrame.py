...
def makeNViewImagesAndCameras():
  ...
  #      ]   
    ...
    for i, image_path in enumerate(image_paths_test):
        ...
        retval_camera.update_after_resize(image_shape_before_resize, [384, 384])
...
def one_epoch(model, config, device, epoch, n_iters_total=0, is_train=True, caption='', experiment_dir=None, writer=None):
    ...
    with grad_context():
        ...
        #batch['pred_keypoints_3d'] = np.zeros([1, 17, 3]);

        nView_batch_images = []
        for batch_images in batch['images']: # n_Views x (Batch数 x H x W x C）の配列から、（BxHxWxC）を順番に処理
            nView_batch_images.append(img.image_batch_to_torch(batch_images).to(device))
        #nView_batch_images.append(image_batch_to_torch(sample['images']).to(device))
        images_batch = torch.stack(nView_batch_images, dim=0) # Batch数 x n_Views x H x W x C のTensorにした

        # Batch数 x n_Views x P (3x4) のTensorにする
        proj_matricies_batch = torch.stack([torch.stack([torch.from_numpy(camera.projection) for camera in camera_batch], dim=0) for camera_batch in batch['cameras']], dim=0).transpose(1, 0).float().to(device)  # shape (batch_size, n_views, 3, 4)
        #proj_matricies_batch = torch.stack([torch.stack([torch.from_numpy(camera.projection) for camera in sample['cameras']], dim=0)], dim=0).transpose(1, 0).float().to(device)  # shape (batch_size, n_views, 3, 4)
        ...
        ######### ALG ##########
        #keypoints_3d_pred_Alg, keypoints_2d_pred_Alg, heatmaps_pred_Alg, confidences_pred_Alg = modelAlg(images_batch, proj_matricies_batch, batch)
        #print(keypoints_3d_pred_Alg)

        ######### VOL ##########
        keypoints_3d_pred, heatmaps_pred, volumes_pred, confidences_pred, cuboids_pred, coord_volumes_pred, ___ = model(images_batch, proj_matricies_batch, batch)
